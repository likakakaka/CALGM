...
def build_optimizer(config, model, simmim=False, is_pretrain=False):
    ...
    if simmim:
        if is_pretrain:
            ...
            parameters = get_finetune_param_groups(model, config.BASE_LR, config.TRAIN.WEIGHT_DECAY, get_layer_func, scales, skip, skip_keywords)
    else:
        parameters = set_weight_decay(model, skip, skip_keywords)
    total=0
    for ii in range(len(parameters)):
        paramm=parameters[ii]['params']
        for jj in range(len(paramm)):
            total += paramm[jj].nelement()
    ...
    return optimizer
# def build_optimizer_cloc(config, model, cloc, simmim=False, is_pretrain=False):
#     """
#     Build optimizer, set weight decay of normalization to 0 by default.
#     """
#     skip = {}
...
#
#     optimizer = None
#     combined_params = cloc.parameters()
#     if opt_lower == 'sgd':
#         optimizer = optim.SGD(list(parameters) + list(cloc.parameters()), momentum=config.TRAIN.OPTIMIZER.MOMENTUM, nesterov=True,
#                               lr=config.BASE_LR, weight_decay=config.TRAIN.WEIGHT_DECAY)
#     elif opt_lower == 'adamw':
#         optimizer = optim.AdamW(combined_params, eps=config.TRAIN.OPTIMIZER.EPS, betas=config.TRAIN.OPTIMIZER.BETAS,
#                                 lr=config.BASE_LR, weight_decay=config.TRAIN.WEIGHT_DECAY)
#     elif opt_lower == 'fused_adam':
#         optimizer = FusedAdam(list(parameters) + list(cloc.parameters()), eps=config.TRAIN.OPTIMIZER.EPS, betas=config.TRAIN.OPTIMIZER.BETAS,
#                               lr=config.BASE_LR, weight_decay=config.TRAIN.WEIGHT_DECAY)
#     elif opt_lower == 'fused_lamb':
#         optimizer = FusedLAMB(list(parameters) + list(cloc.parameters()), eps=config.TRAIN.OPTIMIZER.EPS, betas=config.TRAIN.OPTIMIZER.BETAS,
#                               lr=config.BASE_LR, weight_decay=config.TRAIN.WEIGHT_DECAY)
#
#     return optimizer
...
def set_weight_decay(model, skip_list=(), skip_keywords=()):
    ...
    for name, param in model.named_parameters():
        ...
        # if skip_learnable_name in name:
        #     continue
